...
export function StorageAnalytics({ usage }: { usage: StorageUsageHistory }) {
  ...
  return (
    <div className="bg-white border border-[#ebebeb] rounded-xl p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-gray-900 mb-6">Storage Growth (MB)</h2>
        ...
        {usage.history.map((data, index) => {
          ...
          return (
            <div key={index} className="flex-1 flex flex-col items-center justify-end h-full">
              ...
              <span className="text-xs text-gray-500 mt-2 rotate-45 md:rotate-0 origin-left">{data.month}</span>
            </div>
          );
        })}
      </div>
